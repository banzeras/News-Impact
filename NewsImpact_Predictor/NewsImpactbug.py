...
'''Sem Emoções'''
def intervalo_dados_sem_emotion(m2, intervalo, col_1, col_dest):
    # Create empty lists "X_Fechamento_Ajustado" and y
    X_Fechamento_Ajustado = []
    y = []
    for i in range(len(m2) - intervalo):
        
        # Get close, fake_news_emotion, and target in the loop
        Fechamento_ajustado = m2.iloc[i:(i + intervalo), col_1]
        target = m.iloc[(i + intervalo), col_dest]
        ...
        y.append(target)
    #Criando Variável pra receber um array numpy de X para qualquer target (y)
    vetor = np.hstack(X_Fechamento_Ajustado), np.array(y).reshape(-1, 1)  
    return vetor

A, b = intervalo_dados_sem_emotion(m2, dias_intervalo, col_1, col_dest)

# Divide o dataset em 75% para treinamento
A_split = int(0.75 * len(A))
b_split = int(0.75 * len(b))
...
#Usando o MinMaxScaler() pra alterar a escala dos dados para ficar entre 0 e 1.
A_treino_escala = MinMaxScaler()
A_teste_escala = MinMaxScaler()
b_treino_escala = MinMaxScaler()
b_teste_escala = MinMaxScaler()

# Fit da escala pros conjuntos de treinamento
A_treino_escala.fit(A_treino)
b_treino_escala.fit(b_treino)

# Escala o conjunto de treinamento
A_treino = A_treino_escala.transform(A_treino)
b_treino = b_treino_escala.transform(b_treino)

# Fit da escala pros conjuntos de teste
A_teste_escala.fit(A_teste)
b_teste_escala.fit(b_teste)

# Escala o conjunto de teste
A_teste = A_teste_escala.transform(A_teste)
b_teste = b_teste_escala.transform(b_teste)

# Instanciando o modelo de XGBoost Regressor
modelo2 = XGBRegressor(objective='reg:squarederror', n_estimators=1000)
...
predito2 = modelo2.predict(A_teste)

# Avaliando o modelo
print('Raiz da média do erro quadrado:', np.sqrt(mean_squared_error(y_teste, predito)))
print('R² :', r2_score(y_teste, predito))

print('Sem emotion Raiz da média do erro quadrado:', np.sqrt(mean_squared_error(b_teste, predito2)))
print('Sem emotion R² :', r2_score(b_teste, predito2))

f = open("Avaliação xgbRegressor/Avaliação xgbRegressor "+acao+".txt", "w")
f.write('Raiz da média do erro quadrado: ' + f'{np.sqrt(mean_squared_error(y_teste, predito))}'+
...
preco_predito2 = b_teste_escala.inverse_transform(predito2.reshape(-1, 1))


# Criação de um data frame para armazenar os valores reais e previstos
df_final = pd.DataFrame({"Real": preco_real.ravel(),"Predição": preco_predito.ravel(), "Predição Sem emotion": preco_predito2.ravel()},
